...
describe('Grid Col samples', () => {
  ...
  it('Responsive (tablet)', () => {
    ...
    getEl('col4').then(($col4) => {
      getEl('col5').then(($col5) => {
        assert.equal($col4.position().top, $col5.position().top, 'Col4 and col5 have to be in the same row.')
      })
      getEl('col6').then(($col6) => {
        assert.equal($col4.position().top, $col6.position().top, 'Col4 and col6 have to be in the same row.')
      })
    })
  })
